...
/* Custom link add to cart (popup modal window on single product page) */
/*
	...
	$('.plus_popup').click(function () {
		...
		var $link = $('#add_popup_cart');
		$input.val(parseInt($input.val()) + 1);
		//console.log($final.text() + ' ' + $(this).data('price'));
		$final.text(parseInt($final.text()) + $(this).data('price'));
		$link.attr('href', $(this).data('link') + '&quantity='+ $input.val());
		$input.change(); 
		return false;
...
/* Woo's native add to cart (popup modal window on single page) */
/* updates the input value when quantity changes */ 
    ...
    // get unit price from attribute
    var price = parseFloat($input.attr('data-price')); // safer than .data()
	//console.log('qty:', qty, 'price:', price); //debug

    if (isNaN(price)) price = 0; // fallback

    // calculate total
    var total = (qty * price).toFixed(2);

    // update the total span
    $input.closest('form').find('.result__cost span').text(total);
	});
	...
	*/
	...
	$('.sort-menu li').on('click',function() {
			...
			//console.log(window.location + '?orderby=' + $(this).find('a').data('value'));
			$('#parametr').html($(this).find('a').html());
			//window.location.reload(window.location + '?orderby=' + $(this).find('a').data('value')); 
			window.location = window.location + '?orderby=' + $(this).find('a').data('value');
		});
		*/
	...
	*/
	 ...
	 /*
    //issue - always appending ?orderby=... without checking if a query string already exists. If the URL already has ?orderby=..., this end up stacking parameters incorrectly
    $('.sort-menu li').on('click',function() {
        $('#parametr').html($(this).find('a').html());
        window.location = window.location + '?orderby=' + $(this).find('a').data('value');
        //window.location =  'http://localhost:8000/shop/?orderby=' + $(this).find('a').data('value');
    });
    */
    /*
    //fixed
	// Отключить перезагруску и инитить AJAX фильтры
	/*
	$('.sort-menu li').on('click', function(e) {
		e.preventDefault();

		const orderby = $(this).find('a').data('value');
...
$(document).ready(function(){
	...
	function valEl(el){
		...
		if(!el.length || typeof el.validate !== "function") return;
...
/* Cannot access 'forms' before initialization error 
	const forms = $('form.subscribe__form, .inner__form, .cabinet__form, .review-form, .subscr__form, .order-form, .callme-form').each(function() {
	if (forms.length) {
		forms.each(function() {
		valEl($(this)); 
		});
	...
	*/ 
	...
	const forms = $('form.subscribe__form, .inner__form, .cabinet__form, .review-form, .subscr__form, .order-form, .callme-form'); 
	if (forms.length) {
		forms.each(function() {
			valEl($(this));
		});
